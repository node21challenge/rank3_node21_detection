...
from detectron2.utils.events import CommonMetricPrinter, JSONWriter, CustomTensorboardXWriter
from detectron2.engine import DefaultTrainer, default_argument_parser, default_setup, hooks, launch, ValLossHook, PeriodicWriter
...
def maskr_setup(args):
    """
    Create configs and perform basic setups.
    """
    cfg = get_cfg()
    cfg.merge_from_file(args.config_file_maskr)
...
def retina_setup(args):
    """
    Create configs and perform basic setups.
    """
    cfg = get_cfg()
    cfg.merge_from_file(args.config_file_retina)
    cfg.merge_from_list(args.opts)
    cfg.freeze()
    default_setup(cfg, args)
    return cfg
